...
It **never merges `custom` automatically.** A conflict resolved by a background job is a conflict
resolved badly; the drift report is the trigger for you to do it deliberately.
...
### Merging new upstream into custom
...
Merge rather than rebase: it preserves `custom`'s real history and every original commit SHA, so the
push is an ordinary fast-forward — no force, no rewritten history, no backup branch to hedge against a
bad force-push. `git rerere` is enabled, so a resolution recorded once replays automatically on the
next merge. `git log main..custom` still reads as this fork's delta; the register below mirrors it.
