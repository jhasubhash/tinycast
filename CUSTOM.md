...
| Pin a plugin's nested view to the launcher ("Add to Main Menu") | `TinycastPluginKit/` (`PluginRoute`, `PluginContext.route`, scaffold `route:` + auto "Add to Main Menu" command + `pluginAddToMainMenu` env hook), `Features/Plugins/{Service/PluginManager,UI/PluginCoordinator}.swift` (route through launch/context, `PluginRouteURL`, `pinRoute`/`launchRoute`), `Features/Quicklinks/UI/QuicklinkCoordinator.swift` (intercept `tinycast://plugin/…`), `Palette/RootPaletteView.swift` (inject the env hook) | A saved deep link, stored as an ordinary Quicklink so it inherits aliasing, shortcuts and backup, opens a plugin straight to a nested view. The ⌘K palette auto-offers "Add to Main Menu" on any surface that supplies a `PluginRoute`. Generic seam; plugins support arbitrary views, extensions can adopt command+args later. | Yes |
| "Edit Quicklink" in the launcher's ⌘K menu | `Features/Launcher/UI/AppActionsMenu.swift` | The launcher Actions menu for a quicklink row now offers "Edit Quicklink" (opens the Settings editor for it), matching the Search Quicklinks screen — so a quicklink is editable without first opening Settings by hand. | Yes |
