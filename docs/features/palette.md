...
dock the composer at the bottom with the transcript above it. `windowDidMove` then derives the anchor
from whichever edge is fixed, so an upward resize can't re-anchor to the moving top. Every other
surface keeps the top edge and grows down, exactly as before.
